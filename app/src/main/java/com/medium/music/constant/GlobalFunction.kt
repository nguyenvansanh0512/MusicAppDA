...
object GlobalFunction {
    ...
    fun isFavoriteSong(song: Song): Boolean {
        ...
        for (userInfor in listUsersFavorite) {
            if (DataStoreManager.user?.email == userInfor?.emailUser) {
                return true
            }
        }
        return false
    ...
    fun onClickFavoriteSong(context: Context?, song: Song, isFavorite: Boolean) {
        ...
        if (isFavorite) {
            ...
            val userInfor = UserInfor(System.currentTimeMillis(), userEmail)
            MyApplication[context].getSongsDatabaseReference()
                ?.child(song.id.toString())
                ?.child("favorite")
                ?.child(userInfor.id.toString())
                ?.setValue(userInfor)
        } else {
            val userInfor = getUserFavoriteSong(song)
            if (userInfor != null) {
                MyApplication[context].getSongsDatabaseReference()
                    ?.child(song.id.toString())
                    ?.child("favorite")
